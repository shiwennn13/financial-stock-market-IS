...
def news_home ( request ) :
    ...
    if search :  # Add this block to handle search queries
        url = f"https://newsapi.org/v2/everything?q={search}&language=en&apiKey={api_key}"

    elif category is None or category == "stockmarket":
        # get the top news
        url = f"https://newsapi.org/v2/everything?q=stock-market&language=en&sortBy=publishedAt&apiKey={api_key}"
    else :
